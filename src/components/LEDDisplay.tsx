        ...
        {visibleShipments.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-6 auto-rows-max">
            {visibleShipments.map((shipment) => (
              <div
                key={shipment.id}
                className="bg-slate-800 rounded-lg p-8 border-t-4 border-slate-700 hover:shadow-xl transition-all"
                ...
                <div className="flex flex-col gap-6">
                  <div>
                    ...
                    {shipment.sscc_numbers && (
                      <p className="text-slate-300 text-base ml-11 line-clamp-2">
                        {shipment.sscc_numbers}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-8">
                      ...
                      <div>
                        ...
                        {isArrivingSoon(shipment.start) && (
                          <div className="flex items-center gap-2 mt-2 text-amber-300">
                            ...
                            <span className="font-bold text-lg">Arriving Soon</span>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="text-right">
                      <div className="text-sm text-slate-400 font-medium mb-2">Status</div>
                      <span className={`inline-block px-6 py-3 ${getStatusColor(shipment.status)} text-white rounded-lg font-bold text-lg`}>
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {shipment.assigned_operators.map((op, idx) => (
                            <span
                              key={idx}
                              className="px-4 py-2 bg-slate-700 text-slate-100 rounded-lg text-sm font-semibold"
                            >
                              {op}
                            </span>
                          ))}
